...
def carregar_dados():
    return pd.read_csv(data_file, parse_dates=["Data_Anuncio", "Data_Venda"], dayfirst=True)
...
if pagina == "Dashboard":
    ...
    st.bar_chart(df[df["Preco_Venda"].notna()].groupby(df["Data_Venda"].dt.to_period("M")).sum(numeric_only=True)["Preco_Venda"])
